...
src/research_agent/
  ...
  semantic_scholar.py 呼叫 Semantic Scholar Graph API（無需 API key），回傳含摘要的論文清單
  analyzer.py          將論文清單組成 prompt 送給 Claude API，回傳 markdown 分析內容
  report.py            把分析內容加上 header（日期、論文數、來源）組成最終報告
...
資料流：`search_papers()` -> `analyze()` -> `build_report()` -> 寫入 `<keyword>_report.md`。
...
- 只分析有摘要（abstract）的論文，沒有摘要的論文會在 `semantic_scholar.search_papers()` 被過濾掉。
